...
const Wallet = () => {
  ...
  const deleteWallet = (index, Amount) => {
    ...
    let indexs = alluser.findIndex(
      (user) => user.Accountnum === userAccountnum
    );
  ...
  return (
    <div className="mainwalletcontainer">
      {modal && (
        <form onSubmit={formik.handleSubmit} className="popup form2">
          <h2>Create Wallet</h2>
          <input
          ...
          {formik.touched.walletName && (
            <div className="error">{formik.errors.walletName}</div>
          )}
          <input
            onBlur={formik.handleBlur}
            onChange={formik.handleChange}
      ...
      <div className="walletcontainer">
        {loading &&
          activeUser.wallets.map((wallet, index) => {
            ...
            return (
              <div key={index} className="wallet">
                ...
                <div className="wallet-buttons">
                  <button>Fund</button>
                  <button onClick={() => deleteWallet(index, amount)}>
                    Delete
                  </button>
